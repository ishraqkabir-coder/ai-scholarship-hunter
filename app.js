...
// -- OPENROUTER FETCH ---------------------------------------------------------
async function fetchWithRetry(prompt, attempts){
  attempts = attempts || 3;
  var lastErr;
  for(var i=0; i<attempts; i++){
    ...
    try{
      ...
      if(!res.ok){
        ...
        if(res.status===402) throw new Error('Credits exhausted. Top up OpenRouter account.');
...
async function fetchScholarships(p){
  ...
  try{
    return await fetchWithRetry(prompt, 3);
  }catch(e){
